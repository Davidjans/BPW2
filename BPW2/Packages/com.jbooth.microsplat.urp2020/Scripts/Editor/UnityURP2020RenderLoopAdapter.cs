...
{
   public class UnityURP2020RenderLoopAdapter : IRenderLoopAdapter
   {
      ...
      {
         return "_MSRENDERLOOP_UNITYURP2020";
      }
      ...
      public void Init(string[] paths)
      {
         ...
         {
            string p = paths[i];
            if (p.EndsWith("microsplat_template_urp2020_template.txt"))
            {
            ...
            else if (p.EndsWith("microsplat_template_urp2020_passdepthnormals.txt"))
            {
               templatePassDepthNormals = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_urp2020_passforward.txt"))
            {
               templatePassForward = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_urp2020_passdepthonly.txt"))
            {
               templatePassDepthOnly = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_urp2020_passmeta.txt"))
            {
               templatePassMeta = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_urp2020_passshadows.txt"))
            {
               templatePassShadow = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith ("microsplat_template_urp2020_passgbuffer.txt"))
            {
            ...
            else if (p.EndsWith("microsplat_template_urp2020_vert.txt"))
            {
               templateVert = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_urp2020_include.txt"))
            {
      ...
      StringBuilder BuildTemplate(Blocks blocks, string fallbackOverride = null)
      {
         ...
         passGBuffer.Length = 0; // doesn't work on URP10 yet, delayed until next year
         ...
         header.AppendLine("//   Render Pipeline: URP2020");
